...
class Wb_Original_No_Pic_Db(object):
    ...
    def insertIntoDB(self, obj):
        ...
        try:
            self.cursor.execute('replace into full_user_weibo(wmd5, weiboid, ftime, content, upvotes, forwards, reviews) values(%s, %s, %s, %s, %s, %s, %s)', (obj.wmd5, obj.weiboid, obj.ftime, obj.content, obj.upvotes, obj.forwards, obj.reviews))
            self.conn.commit()
        except Exception, e:
            print 'Mysql Error %d: %s' % (e.args[0], e.args[1])
...
def main():
    wb_o_n_p_d = Wb_Original_No_Pic_Db()
